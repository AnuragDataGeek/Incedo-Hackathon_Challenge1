...
def model2(X_train, X_test, y_train, y_test):
    ...
    parameters = {'kernel': ('linear', 'rbf','poly'), 'C':[0.01,0.1,1],'gamma': [1e-7, 1e-4],'epsilon':[0.1]}
    #svm = NuSVR()
    svr = SVR()
...
if __name__ == "__main__":
    ...
    #print(data.head())
    X_train, X_test, y_train, y_test = train_test_split(data.drop(['Greater_Risk_Probability','Sample_Size'],axis=1),data['Greater_Risk_Probability'], test_size=0.2)
